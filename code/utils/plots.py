...
def last_plot(indices, plot_data, path, epoch, img_res, plot_nimgs):

    if plot_data is not None:
        cam_loc, cam_dir = rend_util.get_camera_for_plot(plot_data['pose'])

        # plot images
        plot_images(plot_data['rgb_eval'], plot_data['rgb_gt'], os.path.join(path, "rendering"), epoch, plot_nimgs, img_res, indices)

        # plot normal maps
        np.save(os.path.join(path, "normal_files", "{:08}_normal.npy".format(indices[0])), plot_data['normal_map'].cpu().numpy())
        plot_normal_maps(plot_data['normal_map'], plot_data['normal_gt'], os.path.join(path, "normal"), epoch, plot_nimgs, img_res, indices)

        # plot depth maps
        np.save(os.path.join(path, "depth_files", "{:08}_depth.npy".format(indices[0])), plot_data['depth_map'].cpu().numpy())
        plot_depth_maps(plot_data['depth_map'], plot_data['depth_gt'], os.path.join(path, "depth"), epoch, plot_nimgs, img_res, indices)

        plot_uncertainty_maps(plot_data['depth_uncertainty_map'], os.path.join(path, "depth_uncertainty"), epoch, plot_nimgs, img_res, indices, "depth_uncertainty")
        plot_uncertainty_maps(plot_data['xyz_normal_uncertainty_map'], os.path.join(path, "normal_uncertainty"), epoch, plot_nimgs, img_res, indices, "xyz_normal_uncertainty")

        mask_save_path = '{0}/{1}_{2}_{3}.png'.format(os.path.join(path, "normal_mask"), "normal_mask", epoch, indices[0])
        mask_save_path_npy = '{0}/{1}_{2}_{3}.npy'.format(os.path.join(path, "normal_mask"), "normal_mask", epoch, indices[0])
        if plot_data['normal_uncer_mask'] is not None:
            normal_mask = ~plot_data['normal_uncer_mask']
            np.save(mask_save_path_npy, normal_mask.cpu().numpy().astype(np.uint8))
            normal_mask = normal_mask.to(torch.int) * 255
            normal_mask = normal_mask.reshape(img_res[0], img_res[1])
            normal_mask = normal_mask.cpu().numpy().astype(np.uint8)
            normal_mask_img = Image.fromarray(normal_mask, mode="L")
            normal_mask_img.save(mask_save_path)

        mask_save_path = '{0}/{1}_{2}_{3}.png'.format(os.path.join(path, "depth_mask"), "depth_mask", epoch, indices[0])
        mask_save_path_npy = '{0}/{1}_{2}_{3}.npy'.format(os.path.join(path, "depth_mask"), "depth_mask", epoch, indices[0])
        if plot_data['depth_uncer_mask'] is not None:
            depth_mask = ~plot_data['depth_uncer_mask']
            np.save(mask_save_path_npy, depth_mask.cpu().numpy().astype(np.uint8))
            depth_mask = depth_mask.to(torch.int) * 255
            depth_mask = depth_mask.reshape(img_res[0], img_res[1])
            depth_mask = depth_mask.cpu().numpy().astype(np.uint8)
            depth_mask_img = Image.fromarray(depth_mask, mode="L")
            depth_mask_img.save(mask_save_path)


def plot_without_mesh(indices, plot_data, path, epoch, img_res, plot_nimgs):
    ...
    if plot_data is not None:
        ...
        plot_uncertainty_maps(plot_data['depth_uncertainty_map'], path, epoch, plot_nimgs, img_res, indices, "depth_uncertainty")
        plot_uncertainty_maps(plot_data['xyz_normal_uncertainty_map'], path, epoch, plot_nimgs, img_res, indices, "xyz_normal_uncertainty")

        if plot_data['depth_uncer_mask'] is not None:
            plot_data['depth_uncer_mask'][0,0]=True
            plot_data['depth_uncer_mask'][-1,-1]=False
            plot_uncertainty_maps(plot_data['depth_uncer_mask'], path, epoch, plot_nimgs, img_res, indices, "depth_mask")
        if plot_data['normal_uncer_mask'] is not None:
            plot_data['normal_uncer_mask'][0,0]=True
            plot_data['normal_uncer_mask'][-1,-1]=False
            plot_uncertainty_maps(plot_data['normal_uncer_mask'], path, epoch, plot_nimgs, img_res, indices, "normal_mask", "inferno")
...
def plot(implicit_network, indices, plot_data, path, epoch, img_res, plot_nimgs, resolution, grid_boundary,  level=0):
    ...
    if plot_data is not None:
        ...
        if plot_data['normal_uncer_mask'] is not None:
            plot_data['normal_uncer_mask'][0,0]=True
            plot_data['normal_uncer_mask'][-1,-1]=False
            plot_uncertainty_maps(plot_data['normal_uncer_mask'], path, epoch, plot_nimgs, img_res, indices, "normal_mask")
...
def plot_normal_maps(normal_maps, ground_true, path, epoch, plot_nrow, img_res, indices):
    ...
    normal_difference = torch.abs(normal_maps - ground_true).mean(dim=-1)[..., None]
    ...
    # save normal difference
    normal_difference = lin2img(normal_difference, img_res)
    normal_difference = normal_difference.expand(-1, 3, -1, -1)
    tensor = torchvision.utils.make_grid(normal_difference,
                                         scale_each=False,
                                         normalize=False,
                                         nrow=plot_nrow).cpu().detach().numpy()
    tensor = tensor.transpose(1, 2, 0)

    save_path = '{0}/normal_difference_{1}_{2}.png'.format(path, epoch, indices[0])
...
def plot_depth_maps(depth_maps, ground_true, path, epoch, plot_nrow, img_res, indices):
    ...
    depth_difference = torch.abs(depth_maps[..., None] - ground_true)
    ...
    depth_difference = lin2img(depth_difference, img_res)
    depth_difference = depth_difference.expand(-1, 3, -1, -1)
    tensor = torchvision.utils.make_grid(depth_difference,
                                         scale_each=False,
                                         normalize=False,
                                         nrow=plot_nrow).cpu().detach().numpy()
    tensor = tensor.transpose(1, 2, 0)

    save_path = '{0}/depth_difference_{1}_{2}.png'.format(path, epoch, indices[0])

    plt.imsave(save_path, tensor[:, :, 0], cmap='viridis')

def plot_uncertainty_maps(uncertainty_maps, path, epoch, plot_nrow, img_res, indices, prefix, cmap = "viridis"):
    uncertainty_maps = uncertainty_maps[..., None]

    uncertainty_maps_plot = lin2img(uncertainty_maps, img_res)
    uncertainty_maps_plot = uncertainty_maps_plot.expand(-1, 3, -1, -1)

    tensor = torchvision.utils.make_grid(uncertainty_maps_plot,
                                         scale_each=False,
                                         normalize=False,
                                         nrow=plot_nrow).cpu().detach().numpy()

    tensor = tensor.transpose(1, 2, 0)
    save_path = '{0}/{1}_{2}_{3}.png'.format(path, prefix, epoch, indices[0])

    plt.imsave(save_path, tensor[:, :, 0], cmap=cmap)
