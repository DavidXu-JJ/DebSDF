...
def evaluate(mesh_pred, mesh_trgt, threshold=.05, down_sample=.02):
    ...
    metrics = {
        ...
        'Recal': recal,
        'F-score': fscore,
        'ChamferL1': (np.mean(dist2) + np.mean(dist1)) / 2
    }
    return metrics
...
all_results = []
for idx, scan in zip(idxes, scenes):
    print(f'Evaluating {scan}')
    ...
    cam_file = f"../data/neural_rgbd_data/scan{idx}/cameras.npz"
    scale_mat = np.load(cam_file)['scale_mat_0']
    mesh.vertices = (scale_mat[:3, :3] @ mesh.vertices.T + scale_mat[:3, 3:]).T

    P = np.load(cam_file)['world_mat_0'] @ scale_mat
    P = P[:3, :4]
    K, poses2 = load_K_Rt_from_P(None, P)

    # load pose and intrinsic for render depth
    pose_path = f'../data/neural_rgbd_data/scan{idx}/poses.txt'
    poses, _ = load_poses(pose_path)
    ...
    gt_mesh = os.path.join(f"../data/neural_rgbd_data/meshes/scan{idx}", "gt_mesh_culled.ply")
    print('GT mesh: {}'.format(gt_mesh))
